 ...
 * ```
 */
export type HasID<T extends UnknownRecord> = T extends { id: unknown }
  ? true
  : false;
 ...
 * ```
 */
export type PrimitiveOrAnyOrRecordWithId<T> = IsAny<T> extends true
  ? T
  : T extends Primitive
    ? T
    : T extends UnknownRecord
      ? HasID<T> extends true
        ? T
        : "❌ Record has no id property"
      : "❌ Not a primitive or record with id";
 ...
 * ```
 */
export type FlattenIds<
  T extends {
    // If the value is an object, here we check if it has an `id` property
 ...
 * ```
 */
export type CreateInput<T extends UnknownRecord> = Omit<
  T,
  "id" | `${string}At`
>;
