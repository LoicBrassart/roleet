...
describe("Scenario entity", () => {
  ...
  it("should allow empty arrays for plans, flashcards, readers, and campaigns", () => {
    ...
    expect(scenario.plans.length).toBe(0);
    expect(scenario.flashcards.length).toBe(0);
    expect(scenario.readers.length).toBe(0);
    expect(scenario.campaigns.length).toBe(0);
  });
});
